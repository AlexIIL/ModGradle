...
public class GenPatchesTask extends AbstractTask {
    ...
    public void genPatches() {
        try {
            ...
            if (MinecraftExtension.genCompilePatches) {
    ...
    public void processFile(String relative, InputStream original, InputStream changed, String name, String patchDir) throws IOException {
        File patchFile = new File(new File(FileLocations.WORKING_DIRECTORY, patchDir), relative + "/" + name + ".patch").getCanonicalFile();

        if (changed == null) {
            getLogger().error("Changed File does not exist");
            return;
        }
        ...
        if (!diff.isEmpty()) {
            ...
            if (!olddiff.equals(unidiff)) {
                getLogger().info("Writing patch: " + patchFile);
                patchFile.getParentFile().mkdirs();
                Files.touch(patchFile);
                Files.write(unidiff, patchFile, Charsets.UTF_8);
            } else {
                getLogger().info("Patch did not change");
            }
        }
    }
}
